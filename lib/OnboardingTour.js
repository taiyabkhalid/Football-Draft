...
export default function OnboardingTour({ slides, forced, onComplete, onSkip }) {
  ...
  return (
    ...
    >
      <div
        className="bg-df-surface rounded-xl p-5 text-center flex flex-col"
      ...
      >
        <div className="flex-1 flex flex-col items-center justify-center" style={{ minHeight: 230 }}>
          {slide.visual ? (
            ...
            <div
              ...
              style={{ width: 44, height: 44, background: slide.iconBg || 'var(--df-info-bg)' }}
            >
              <i className={`ti ${slide.icon}`} style={{ fontSize: 22, color: slide.iconColor || 'var(--df-accent)' }} aria-hidden="true" />
            </div>
          )}
          <p className="text-[15px] font-medium m-0 mb-1.5" style={{ color: 'var(--df-text-primary)' }}>
            {slide.title}
          </p>
          <p className="text-[13px] m-0" style={{ color: 'var(--df-text-muted)', lineHeight: 1.5 }}>
            {slide.body}
          </p>
        </div>
        {slides.length > 1 && (
            ...
            {slides.map((_, i) => (
              <span
                ...
                style={{
                  ...
                  height: 6,
                  borderRadius: '50%',
                  background: i === index ? 'var(--df-accent)' : 'var(--df-border)',
                }}
              />
            ))}
          </div>
        ...
        <div className="flex items-center justify-between">
          {!forced ? (
            <button onClick={onSkip} className="text-xs" style={{ color: 'var(--df-text-faint)' }}>
              Skip
            </button>
          ) : index > 0 ? (
            <button onClick={() => setIndex((i) => i - 1)} className="text-xs" style={{ color: 'var(--df-text-faint)' }}>
              Back
            </button>
          ) : (
            <span />
          ...
          <button
            ...
            style={{ background: 'var(--df-accent)', color: 'var(--df-surface)', border: 'none', padding: '7px 16px' }}
          >
            {isLast ? 'Got it' : 'Next'}
          </button>
        </div>
