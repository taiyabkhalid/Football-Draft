...
export default function HeadshotCapture({ onPhotoReady, initialUrl }) {
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      // Requesting a strict square aspect ratio here was forcing the camera
      // to crop its natural, wider field of view down to fit 1:1 before it
      // even reached the browser - which looks exactly like being zoomed
      // in too far. The circular preview frame already crops visually via
      // CSS (object-fit: cover) further down, so the raw stream itself
      // doesn't need to be pre-cropped - letting it capture its full native
      // view and cropping only on the display side gives a wider, more
      // natural result. Where the browser exposes a zoom control, explicitly
      // setting it to the minimum (rather than zooming in) keeps the widest
      // possible view instead of narrowing it further.
      ...
      if (track && typeof track.getCapabilities === 'function') {
        try {
          ...
          // Not all devices/browsers support this - the shot still works
          // fine without it, just without the extra distortion correction.
        }
      }
  ...
  return (
    <div className="flex flex-col items-center bg-surface rounded-lg p-4">
      {mode === 'idle' && (
        <div
          className="w-40 h-40 rounded-full border-2 border-dashed border-line flex items-center justify-center mb-3"
          aria-hidden="true"
        >
      ...
      {mode === 'camera' && (
        <div className="relative w-44 h-44 mb-3 overflow-hidden rounded-full border-2 border-royal bg-df-surface">
          <video
            ref={videoRef}
            autoPlay
            playsInline
      ...
      {mode === 'idle' && (
        <p className="text-xs text-muted mb-3 text-center px-2">
          GMs will need a clear picture of who you are to help with drafting.
        </p>
      )}
