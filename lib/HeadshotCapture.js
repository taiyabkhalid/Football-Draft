...
export default function HeadshotCapture({ onPhotoReady }) {
  ...
  return (
    <div className="flex flex-col items-center bg-field-800 rounded-lg p-4">
      ...
      {mode === 'camera' && (
        <div className="relative w-40 h-52 mb-3 overflow-hidden rounded-[50%] border-2 border-lights bg-field-900">
          <video
            ref={videoRef}
            autoPlay
            playsInline
      ...
      {mode === 'preview' && previewUrl && (
        <img
          ...
          className="w-32 h-32 rounded-full object-cover border-2 border-lights mb-3"
        />
      )}
