...
// where a skip happened (player_id is null), ascending. Recursive - a
// skip that itself happened during the extended phase (a makeup pick
// that was itself skipped) resolves through the same logic again rather
// than the plain rotation formula, which only works for normal-phase picks.
export function getTeamOnTheClockExtended(clockPickNumber, numTeams, teams, draftType, poolSize, skipPickNumbers) {
  ...
  return getTeamOnTheClockExtended(skipPickNumber, numTeams, teams, draftType, poolSize, skipPickNumbers);
}
...
export function getExtendedRound(clockPickNumber, numTeams, teams, draftType, poolSize, skipPickNumbers) {
  ...
  for (let i = 1; i <= nthSkip; i++) {
    ...
    const skipTeam = getTeamOnTheClockExtended(skipPickNum, numTeams, teams, draftType, poolSize, skipPickNumbers);
    const skipTeamId = skipTeam?.id;

    let candidateRound = floorRound;
    for (;;) {
      let isFree = true;
    ...
    floorRound = Math.max(floorRound, candidateRound);
