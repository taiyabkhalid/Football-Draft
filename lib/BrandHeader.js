...
export default function BrandHeader({ pageLabel, liveIndicator, pickTimer, showSoundToggle, soundOn, onToggleSound }) {
  ...
  if (isGmOrCommish) {
    ...
    navLinks.push({ label: 'Watch Draft', href: '/live', isWatchDraftLink: true });
  }
            ...
            return (
              <a
                ...
                onClick={(e) => {
                  ...
                  router.push(`${link.href}${separator}t=${Date.now()}`);
                }}
                style={{
                  display: 'flex',
                  alignItems: 'center',
              ...
              >
                {link.label}
                {link.isWatchDraftLink && isDraftLive && (
                  <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#3b6d11' }} />
                    <span style={{ fontSize: 11, color: '#3b6d11', fontWeight: 600 }}>Live</span>
                  </span>
                )}
              </a>
            );
