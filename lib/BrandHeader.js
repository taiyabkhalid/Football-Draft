...
export default function BrandHeader({ pageLabel, liveIndicator, pickTimer, showSoundToggle, soundOn, onToggleSound }) {
  ...
  async function handleToggleDarkMode() {
    ...
    // BrandHeader only manages its own local copy of this preference -
    // each page fetches and holds its own separate darkModeEnabled state
    // to theme its own root element, with no other link between the two.
    // This event is how the toggle actually reaches whichever page is
    // currently rendering it, without needing a full shared-state refactor.
    window.dispatchEvent(new CustomEvent('df-dark-mode-changed', { detail: { enabled: newValue } }));
  }
