...
export default function MondayPicker({ selected, onChange }) {
  ...
  return (
    <div>
      <div className="bg-surface border border-line rounded-lg p-3 max-w-sm">
        <div className="flex items-center justify-between mb-2">
          <button
            type="button"
            onClick={goPrevMonth}
            disabled={isCurrentMonth}
            className="text-muted hover:text-royal disabled:opacity-20 disabled:hover:text-muted px-2 text-sm"
          ...
          <button
            type="button"
            onClick={goNextMonth}
            className="text-muted hover:text-royal px-2 text-sm"
        ...
        <div className="grid grid-cols-7 gap-1 text-center text-xs">
          {cells.map((date, i) => {
            ...
            if (!isMonday) {
              return (
                <span key={i} className="py-1.5 text-faint/70">
                  {date.getDate()}
                </span>
              );
            }
              ...
              <button
                ...
                className={`rounded-full py-1.5 font-medium ${
                  isPast
                    ...
                    : active
                    ? 'bg-royal text-white'
                    : 'text-ink hover:bg-line/40 border border-line'
                }`}
              >
                {date.getDate()}
              </button>
